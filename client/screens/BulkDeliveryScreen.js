...
const BulkDeliveryScreen = ({ navigation }) => {
    ...
    const [actionAndLocationType, setActionAndLocationType] = useState("");
    ...
    useEffect(() => {
        setBarcodes((prevBarcodes) => {
            ...
            return [...prevBarcodes, ...Array(noOfBarcodes - prevBarcodes.length).fill({ barcode: "", status: "", type: "", currTrackingStatus: "" })];
        });
    }, [noOfBarcodes]);

    useEffect(() => {
        if (actionType && location) {
            setActionAndLocationType(locations.find((item) => item.value === location)?.type + actionType);
        }
    }, [actionType, location]);
    ...
    const InputData = () => {
        ...
        if (actionAndLocationType === "warehousepickup" || actionAndLocationType === "plantpickup") {
            input = (<>
    ...
    const locations = [
        { label: "Hosakote", type: "plant", value: "svsg_hosakote" },
        { label: "Peenya", type: "warehouse", value: "svsg_peenya" },
        { label: "Client", type: "client", value: "client" },
    ]
